...
function wpjb_event_expiring_jobs() 
{
    ...
    $dt = date_i18n("Y-m-d", strtotime("today +5 day"));
...
function wpjb_event_subscriptions_daily() {
    ...
    $query->where("last_run < ?", date_i18n("Y-m-d H:i:s", strtotime("now -1 day")));
    ...
    foreach($result as $alert) {
        ...
        $params["date_to"] = date_i18n("Y-m-d H:i:s");
        $params["query"] = $params["keyword"];
        
        if($params["date_from"] == "0000-00-00 00:00:00") {
            $params["date_from"] = date_i18n("Y-m-d H:i:s", strtotime("now -1 day"));
        }
        ...
        $mail->assign("unsubscribe_url", wpjb_link_to("alertdelete", $alert, array(
...
function wpjb_event_subscriptions_weekly() {
    ...
    $query = Daq_Db_Query::create();
    $query->from("Wpjb_Model_Alert t");
    $query->where("last_run < ?", date_i18n("Y-m-d H:i:s", strtotime("now -7 day")));
    ...
    foreach($result as $alert) {
        ...
        if($params["date_from"] == "0000-00-00 00:00:00") {
            $params["date_from"] = date_i18n("Y-m-d H:i:s", strtotime("now -7 day"));
        }
